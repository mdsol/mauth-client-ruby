...
describe MAuth::Client do
  describe '.default_config' do
    ...
    it 'raises when it cannot find mauth config yml' do
      expect { MAuth::Client.default_config }.to raise_error(MAuth::Client::ConfigurationError)
    end

    it 'has root option specified' do
      MAuth::Client.default_config(:root => 'spec/config_root')['app_uuid'].should == 'NORMAL-DEVELOPMENT'
    end
    ...
    it 'has environment option specified' do
      Dir.chdir('spec/config_root') do
        MAuth::Client.default_config(:environment => 'production')['app_uuid'].should == 'NORMAL-PRODUCTION'
      end
    end
      ...
      Dir.chdir('spec/config_root') do
        %w(RAILS_ENV RACK_ENV).each do |var|
          with_env(var => 'production') do
            MAuth::Client.default_config['app_uuid'].should == 'NORMAL-PRODUCTION'
          end
        end
      end
    end
    ...
    it 'has mauth_config_yml option specified' do
      MAuth::Client.default_config(:mauth_config_yml => 'spec/config_root/config/mauth.yml')['app_uuid'].should == 'NORMAL-DEVELOPMENT'
    end
    ...
    it 'has logger option specified' do
      Dir.chdir('spec/config_root') do
        ...
        MAuth::Client.default_config(:logger => logger)['logger'].should == logger
      end
    end
