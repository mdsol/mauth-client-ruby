...
When doing local authentication, rack-mauth will periodically fetch shared secrets from mAuth.  Each token has it's own refresh timer for 60 seconds. Applications which connect frequently benefit the most from this caching strategy. If mAuth is unable to respond, or if its response is a 500 status code, the token in the local cache will get a freshness extension for another 60 seconds. When the response is a 200 status code, rack-mauth will parse the security token from mAuth and add it to the local cache. If mAuth responds with 404, it is an indication that mAuth doesn't know that application. Therefore, that incoming request should not be honored and the corresponding token is removed from the local cache by rack-mauth.
...
More information regarding rack-mauth is available at: https://sites.google.com/a/mdsol.com/knowledgebase/home/departments/engineering/on-demand-portfolio/services/core-services/mauth/rack-mauth